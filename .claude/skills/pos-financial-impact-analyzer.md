    ...
    lines: [
      {
        ...
        debit: totalRevenue,
        credit: 0,
        description: sale.customerId ? 'Receivable from customer' : 'Cash received'
      },
      {
...
async function generateAdjustmentGLEntry(
  ...
  return {
    ...
    description: `Inventory ${isShortage ? 'shortage' : 'overage'} - ${correction.product.name}`,
    lines: isShortage ? [
...
export async function GET(request: NextRequest) {
  ...
  for (const sale of sales) {
    const saleEntries = await generateSaleGLEntry(sale.id, session.user.businessId)
...
function exportToQuickBooksIIF(entries: JournalEntry[]): string {
  ...
  for (const entry of entries) {
    for (const line of entry.lines) {
      ...
      iif += `TRNS\t\tGENERAL JOURNAL\t${formatDate(entry.entryDate)}\t${line.account}\t\t${amount}\t${entry.description}\n`
    }
    iif += 'ENDTRNS\n'
  }
...
function exportToCSV(entries: JournalEntry[]): string {
  const rows = [['Date', 'Type', 'Reference', 'Account', 'Debit', 'Credit', 'Description']]
