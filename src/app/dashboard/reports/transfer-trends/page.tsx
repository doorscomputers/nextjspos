...
export default function TransferTrendsPage() {
  ...
  useEffect(() => {
    fetchTrends()
  }, [year, groupBy, productId, fromLocationId, toLocationId])
  ...
  const fetchTrends = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/transfers/trends?${params.toString()}`, {
        credentials: 'include',
        headers: {
      ...
      console.error('Failed to fetch transfer trends:', error)
      setError(error instanceof Error ? error.message : 'Failed to load transfer trends')
      setTrendData(null)
    } finally {
      setLoading(false)
    }
  ...
  const exportToCSV = () => {
    ...
    const rows = trendData.products.map((product) => [
      ...
      ...trendData.allPeriods.map(period => product.byPeriod[period] || 0)
    ])
    ...
    a.href = url
    a.download = `transfer-trends-${year}-${groupBy}-${format(new Date(), 'yyyy-MM-dd')}.csv`
    a.click()
  ...
  const exportToExcel = () => {
    ...
    const worksheetData = [
      ["Product Name", "SKU", "Total Quantity", "Transfers", ...trendData.allPeriods],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Transfer Trends")
    XLSX.writeFile(workbook, `transfer-trends-${year}-${groupBy}-${format(new Date(), 'yyyy-MM-dd')}.xlsx`)
  }
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(18)
    doc.text(`Transfer Trends Report - ${year}`, 14, 15)
    doc.setFontSize(10)
    ...
    doc.text(`Total Quantity Transferred: ${trendData.summary.totalQuantityTransferred}`, 14, 36)

    // Top 10 table
    autoTable(doc, {
      startY: 42,
      head: [['Product', 'SKU', 'Total Qty', 'Transfers']],
    ...
    doc.save(`transfer-trends-${year}-${groupBy}-${format(new Date(), 'yyyy-MM-dd')}.pdf`)
  }
  ...
  const chartDataFormatted = trendData?.periodComparison?.map((period, index) => ({
    ...
    ...(trendData.top10Products || []).reduce((acc, product) => {
      acc[product.productName] = product.byPeriod?.[period.period] || 0
      return acc
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between no-print">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Transfer Trends Analysis</h1>
          <p className="text-sm text-gray-600 mt-1">
            Analyze product transfer patterns over time with visual comparisons
          </p>
        </div>
      ...
      <Card className="no-print">
        ...
        <CardContent className="space-y-4">
          ...
          {(productId !== 'all' || fromLocationId !== 'all' || toLocationId !== 'all') && (
            <div className="flex items-center gap-2 pt-2 border-t border-gray-200">
              ...
              {fromLocationId !== 'all' && (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                  From: {locations.find(l => l.id.toString() === fromLocationId)?.name}
                  <button
                    onClick={() => setFromLocationId('all')}
              ...
              {toLocationId !== 'all' && (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                  To: {locations.find(l => l.id.toString() === toLocationId)?.name}
                  <button
                    onClick={() => setToLocationId('all')}
      ...
      {trendData && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 no-print">
          ...
          <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            ...
            <CardContent>
              <p className="text-3xl font-bold text-green-900">{trendData.summary.totalQuantityTransferred.toLocaleString()}</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-orange-700">Total Transfers</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold text-orange-900">{trendData.summary.totalTransfers}</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-purple-700">Avg Qty/Transfer</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold text-purple-900">{trendData.summary.avgQuantityPerTransfer}</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      {loading ? (
        ...
        <Card className="no-print">
          <CardHeader>
            <CardTitle>Top 10 Products - Transfer Trends ({year})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-96">
      ...
      {trendData && (
        <Card id="print-area">
          <CardHeader>
            <CardTitle>Product Transfer Details</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b-2 border-gray-300 bg-gray-50">
                    ...
                    <th className="text-right p-3 font-semibold text-gray-700">Avg Qty/Transfer</th>
                  </tr>
                </thead>
                <tbody>
                  {trendData.products.map((product, index) => (
                    <tr key={product.productId} className="border-b border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="p-3 text-right text-gray-800">{product.transferCount}</td>
                      <td className="p-3 text-right text-gray-700">
                        {(product.totalQuantity / product.transferCount).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
