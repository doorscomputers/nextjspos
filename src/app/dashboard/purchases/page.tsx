...
export default function PurchasesDevExtremePageOptimized() {
  const { can } = usePermissions()
  const [refreshKey, setRefreshKey] = useState(0)
  const [statusFilter, setStatusFilter] = useState<string>('pending') // Default to pending

  // ✅ OPTIMIZATION: Create DevExtreme CustomStore for server-side operations
  const dataSource = useCallback(() => {
    return new CustomStore({
      key: 'id',
      load: async (loadOptions) => {
        try {
          console.log('⏱️ [PURCHASES] Loading data with options:', loadOptions)
          console.time('⏱️ [PURCHASES] Fetch time')

          // Build query parameters from DevExtreme load options
          ...
          // Include details for master-detail view
          params.append('includeDetails', 'true')

          // Sorting
          if (loadOptions.sort) {
            const sortInfo = loadOptions.sort[0]
            params.append('sortBy', sortInfo.selector)
            params.append('sortOrder', sortInfo.desc ? 'desc' : 'asc')
          }

          // Filtering (DevExtreme filter expressions)
          if (loadOptions.filter) {
            // Convert DevExtreme filter to API format
            // For now, we'll handle basic filters
            // More complex filters can be added based on requirements
            params.append('filter', JSON.stringify(loadOptions.filter))
          }

          const url = `/api/purchases?${params.toString()}`
          console.log('📡 [PURCHASES] Fetching:', url)

          const response = await fetch(url)
          console.timeEnd('⏱️ [PURCHASES] Fetch time')
          ...
          console.log(`✅ [PURCHASES] Loaded ${data.purchases.length} purchases (Page ${page}/${data.pagination.totalPages})`)
          ...
          console.error('❌ [PURCHASES] Failed to fetch:', error)
  ...
  const handleStatusFilterChange = (value: string) => {
    console.log('🔄 [PURCHASES] Status filter changed to:', value)
          ...
          <div className="bg-white dark:bg-amber-900/30 rounded-lg shadow p-4 border border-amber-200 dark:border-amber-700">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tfoot className="bg-amber-50 dark:bg-amber-900/50">
                  <tr>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50/30 to-amber-50 dark:from-amber-950 dark:via-orange-900 dark:to-amber-950 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-amber-700 dark:text-amber-300 mt-1">
            Manage purchase orders with server-side pagination
          </p>
        </div>
      ...
      {/* Status Filter */}
      <div className="bg-white dark:bg-amber-900/30 rounded-lg shadow-md p-4 border border-amber-200 dark:border-amber-700">
      ...
      {/* DevExtreme DataGrid with Remote Operations */}
      <div className="bg-white dark:bg-amber-900/30 rounded-lg shadow-lg p-6 border border-amber-200 dark:border-amber-700">
        <DataGrid
          ...
          height={700}
        >
          {/* ✅ OPTIMIZATION: Enable remote operations for server-side processing */}
          <RemoteOperations
            paging={true}
            sorting={true}
            filtering={false} // We handle filtering via status dropdown
          />
