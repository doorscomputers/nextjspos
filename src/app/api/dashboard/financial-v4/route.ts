...
export async function GET(request: NextRequest) {
  try {
    ...
    const allProductIds = [
      ...new Set([
        ...topSellingByQty.map(item => item.productId),
        ...topGrossingByProfit.map(item => item.productId)
      ])
    ]
    ...
    // Process top selling products in memory (no DB queries)
    const topSellingProducts = topSellingByQty.map((item) => {
    ...
    // Process products with profit calculations in memory (no DB queries)
    const productsWithProfit = topGrossingByProfit.map((item) => {
      const product = productMap.get(item.productId)
