...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ OPTIMIZED: Pre-load unit information only when needed for transactions
    // This prevents over-fetching for non-POS queries
    ...
    // OPTIMIZED: Only load unit prices for transaction forms (POS, purchases, etc.)
    // This reduces data transfer by 30-50% for product list queries
    if (forTransaction) {
      productInclude.unitPrices = {
        select: {
          unitId: true,
          purchasePrice: true,
          sellingPrice: true,
      ...
      productInclude.unitLocationPrices = {
        select: {
          locationId: true,
          unitId: true,
          purchasePrice: true,
