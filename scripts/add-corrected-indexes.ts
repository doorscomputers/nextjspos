...
async function addCorrectedIndexes() {
    console.log('🚀 Adding corrected performance indexes...\n')
    ...
    const indexes = [
        ...
        // ============================================
        // SALES TABLE INDEXES (Corrected - no payment_status)
        // ============================================
        {
        ...
        // Covering index for dashboard stats
        {
            ...
            sql: `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sales_dashboard_covering 
            ON sales(business_id, sale_date DESC, location_id) 
    ...
    for (const index of indexes) {
        try {
            ...
            if (error.message?.includes('already exists')) {
                ...
                console.log(`⚠️  Skipping ${index.name} - column doesn't exist`)
                skippedCount++
            } else {
