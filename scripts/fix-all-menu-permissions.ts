...
async function fixAllMenuPermissions() {
  console.log('🔧 FIXING ALL MENU PERMISSIONS - COMPREHENSIVE FIX\n')
  console.log('='
...
.repeat(80))
  ...
  try {
    ...
    const menuKeyToId = new Map<string, number>()

    let created = 0
    let skipped = 0
    let linksCreated = 0

    // Step 2: Process all menus in order (parents first)
    console.log('📋 Processing All Menu Records:\n')

    for (const menuDef of ALL_SIDEBAR_MENUS) {
      // Check if menu exists
      let menu = await prisma.menuPermission.findFirst({
        where: { key: menuDef.key }
      })

      if (!menu) {
        // Need to create it - find parent ID first
        let parentId: number | null = null
        if (menuDef.parentKey) {
          parentId = menuKeyToId.get(menuDef.parentKey) || null
        }

        menu = await prisma.menuPermission.create({
          data: {
            key: menuDef.key,
            name: menuDef.name,
            href: menuDef.href,
        ...
        console.log(`   ✅ Created: ${menuDef.key}`)
        created++
      } else {
        console.log(`   ⏭️  Exists: ${menuDef.key}`)
        skipped++
      }

      // Store ID for children to reference
      menuKeyToId.set(menuDef.key, menu.id)

      // Step 3: Link to ALL roles
      for (const role of allRoles) {
    ...
    console.log('📊 Summary:')
    console.log(`   ✅ Menu records created: ${created}`)
    console.log(`   ⏭️  Menu records already existed: ${skipped}`)
    console.log(`   🔗 Role-menu links created: ${linksCreated}`)
    console.log(`   📋 Total menus processed: ${ALL_SIDEBAR_MENUS.length}`)
    console.log()
    console.log('📝 Next Steps:')
    console.log('   1. ALL users must LOGOUT and LOGIN again')
