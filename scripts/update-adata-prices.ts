/**
 * Script to update ADATA product costs and selling prices from Excel
 * Run with: DATABASE_URL="postgresql://postgres.ydytljrzuhvimrtixinw:Mtip12_14T%21@aws-1-ap-southeast-1.pooler.supabase.com:5432/postgres" npx tsx scripts/update-adata-prices.ts
 */
...
const EXCEL_FILE_PATH = 'c:/Users/Warenski/Downloads/ADATA (PRICE UPDATE for system).xlsx'
...
async function main() {
  ...
  for (const row of data) {
    ...
    const itemName = String(row[' PRODUCT '] || row['PRODUCT'] || '').trim()
    const newCost = row[' NEW COST '] || row['NEW COST']
    const newPrice = row[' NEW SRP '] || row['NEW SRP']
