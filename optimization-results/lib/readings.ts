 ...
 * @param incrementCounter - Whether to increment X reading counter (default: { select: { id: true, name: true } })
 ...
 * @param businessId - Business ID for multi-tenant isolation
 * @param cashierName - Name of the cashier
 * @param incrementCounter - Whether to increment Z counter (default: { select: { id: true, name: true } })
...
export async function generateZReadingData(
  ...
  const shift = await prisma.cashierShift.findFirst({
    ...
    select: {
      sales: {
        select: {
          payments: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } },
          items: { select: { id: true, name: true } }, // Just items, we'll fetch products separately
        },
      },
      cashInOutRecords: { select: { id: true, name: true } },
      cashDenominations: {
  ...
  const business = await prisma.business.findUnique({
    where: { id: businessId },
    select: {
      zCounter: { select: { id: true, name: true } },
      resetCounter: { select: { id: true, name: true } },
      accumulatedSales: { select: { id: true, name: true } },
      lastZReadingDate: { select: { id: true, name: true } },
      name: { select: { id: true, name: true } },
