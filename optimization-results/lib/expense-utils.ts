...
export async function generateExpenseReferenceNumber(
  ...
  const lastExpense = await prisma.expense.findFirst({
    ...
    select: {
      referenceNumber: { select: { id: true, name: true } },
    },
  });
  ...
  try {
    ...
    const journalEntry = await prisma.$transaction(async (tx) => {
      ...
      const paymentAccount = await tx.chartOfAccounts.findUnique({
        ...
        select: { accountType: { select: { id: true, name: true } }, normalBalance: { select: { id: true, name: true } } },
      });
    ...
    return { success: { select: { id: true, name: true } }, journalEntryId: journalEntry.id };
  } catch (error) {
  ...
  try {
    const expense = await prisma.expense.findUnique({
      ...
      select: {
        journalEntry: {
          select: {
            lines: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } },
          },
        },
      },
    });
    ...
    if (!expense.journalEntry) {
      ...
      return { success: { select: { id: true, name: true } } };
    }
      ...
      for (const line of expense.journalEntry.lines) {
        ...
        if (line.credit > 0) {
          const account = await tx.chartOfAccounts.findUnique({
            where: { id: line.accountId },
            select: { normalBalance: { select: { id: true, name: true } } },
          });
