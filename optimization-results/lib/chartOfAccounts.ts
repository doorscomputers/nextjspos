...
export const STANDARD_CHART_OF_ACCOUNTS = [
  ...
  // ============================================
  {
    ...
    isSystemAccount: { select: { id: true, name: true } },
  },
  {
    accountCode: '1050',
...
export async function getActiveAccounts(businessId: number) {
  return await prisma.chartOfAccounts.findMany({
    where: {
      businessId,
      isActive: { select: { id: true, name: true } },
      allowManualEntry: { select: { id: true, name: true } }, // Only show accounts that allow manual entries
    },
    select: {
      ...
      normalBalance: { select: { id: true, name: true } },
    },
    orderBy: { accountCode: 'asc' },
  })
}
