  ...
  const [
    ...
    prisma.saleItem.findMany({
      ...
      select: {
        sale: {
          select: {
            customer: { select: { name: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } } } }
          }
        }
      },
      orderBy: {
    ...
    prisma.inventoryCorrection.findMany({
      ...
      select: {
        stockTransaction: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } }  // Include the linked stock transaction to get actual adjustment
      },
      orderBy: { approvedAt: 'asc' }
    }),
...
export async function verifyAndCorrectStock(
  ...
  if (!latestTransaction) {
    ...
    if (currentQty !== 0) {
      ...
      return {
        corrected: { select: { id: true, name: true } },
        oldQty: currentQty,
        newQty: 0,
