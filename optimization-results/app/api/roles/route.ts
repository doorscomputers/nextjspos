...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: { select: { id: true, name: true } }, data: formattedRoles })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: { select: { id: true, name: true } }, role }, { status: 201 })
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const role = await prisma.role.findFirst({
      ...
      select: {
        users: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } },
      },
    })
    ...
    return NextResponse.json({ success: { select: { id: true, name: true } } })
  } catch (error) {
