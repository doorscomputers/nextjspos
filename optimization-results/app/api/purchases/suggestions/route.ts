...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = await prisma.product.findMany({
      where: {
        businessId,
        ...(onlyEnabled ? { enableAutoReorder: { select: { id: true, name: true } } } : {}),
        ...(categoryId ? { categoryId: parseInt(categoryId) } : {}),
        type: 'single', // Only single products for now
        enableStock: { select: { id: true, name: true } }, // EXCLUDE non-inventory products (services, etc.)
        isActive: { select: { id: true, name: true } }, // EXCLUDE inactive products
      },
      select: {
        category: {
