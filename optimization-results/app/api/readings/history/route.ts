  ...
  try {
    ...
    const [shifts, users, locations] = await Promise.all([
      prisma.cashierShift.findMany({
        ...
        select: { id: { select: { id: true, name: true } }, shiftNumber: { select: { id: true, name: true } } },
      }),
...
async function hydrateShiftReadings(businessId: number, locationIds: number[]) {
  ...
  const shiftsNeedingLogs = await prisma.cashierShift.findMany({
    ...
    select: {
      ...
      shiftNumber: { select: { id: true, name: true } },
      openedAt: { select: { id: true, name: true } },
      closedAt: { select: { id: true, name: true } },
      updatedAt: { select: { id: true, name: true } },
      totalSales: { select: { id: true, name: true } },
      totalDiscounts: { select: { id: true, name: true } },
      systemCash: { select: { id: true, name: true } },
      transactionCount: { select: { id: true, name: true } },
      xReadingCount: { select: { id: true, name: true } },
    },
    take: 50,
  })
