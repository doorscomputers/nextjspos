  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const stockResult = await updateStock({
        ...
        allowNegative: { select: { id: true, name: true } },
      })
      ...
      const updatedCorrection = await tx.inventoryCorrection.update({
        ...
        select: {
          ...
          productVariation: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } }, sku: { select: { id: true, name: true } } } },
          location: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } },
          approvedByUser: { select: { username: { select: { id: true, name: true } }, firstName: { select: { id: true, name: true } }, lastName: { select: { id: true, name: true } } } },
        },
      })
