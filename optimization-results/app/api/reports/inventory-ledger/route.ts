...
export async function GET(request: NextRequest) {
  try {
    ...
    const lastCorrection = await prisma.inventoryCorrection.findFirst({
      ...
      select: {
        createdByUser: {
          select: { username: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } } }
        }
      }
    })
      ...
      const [
        ...
        prisma.stockTransfer.findMany({
          where: {
            ...
            stockDeducted: { select: { id: true, name: true } },
            sentAt: { lt: startDate },
            items: {
              some: {
                productId: prodId,
    ...
    const [
      ...
      prisma.purchaseReceipt.findMany({
        ...
        select: {
          ...
          purchase: {
            select: { purchaseOrderNumber: { select: { id: true, name: true } } }
          }
        },
      ...
      prisma.stockTransfer.findMany({
        where: {
          ...
          stockDeducted: { select: { id: true, name: true } }, // Only transfers where stock was actually deducted
          sentAt: {
            gte: startDate,
            lte: endDate
          },
      ...
      prisma.customerReturn.findMany({
        ...
        select: {
          ...
          sale: {
            select: { invoiceNumber: { select: { id: true, name: true } } }
          }
        },
        orderBy: { approvedAt: 'asc' }
      }),
