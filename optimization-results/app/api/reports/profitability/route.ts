    ...
    const [products, variations, locationsData] = await Promise.all([
      productIds.size > 0
        ? prisma.product.findMany({
            ...
            select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } }, categoryId: { select: { id: true, name: true } } },
          })
        : [],
      variationIds.size > 0
        ? prisma.productVariation.findMany({
            where: { id: { in: Array.from(variationIds) } },
            select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } }, productId: { select: { id: true, name: true } } },
          })
        : [],
      locationIds.size > 0
