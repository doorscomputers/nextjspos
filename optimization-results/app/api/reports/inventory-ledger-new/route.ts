  ...
  try {
    ...
    if (startDateParam) {
      const lastTransactionBeforeStart = await prisma.stockTransaction.findFirst({
        ...
        select: {
          balanceQty: { select: { id: true, name: true } },
    ...
    const stockTransactions = await prisma.stockTransaction.findMany({
      ...
      select: {
        createdByUser: {
          select: {
            username: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } },
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        metadata: {
          ...
          noDuplicateGuarantee: { select: { id: true, name: true } },
