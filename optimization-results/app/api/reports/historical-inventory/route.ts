    ...
    const [inventoryData, totalCount] = await Promise.all([
      ...
      prisma.variationLocationDetails.findMany({
        ...
        select: {
          product: {
            select: {
              category: { select: { name: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } } } },
              brand: { select: { name: { select: { id: true, name: true } } } }
            }
          },
          productVariation: {
            select: {
              supplier: { select: { name: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } } } }
            }
          }
        },
        orderBy: [
    ...
    const inventoryWithHistoricalQty = await Promise.all(
      inventoryData.map(async (item) => {
        ...
        const lastTx = await prisma.stockTransaction.findFirst({
          ...
          select: { balanceQty: { select: { id: true, name: true } }, unitCost: { select: { id: true, name: true } }, createdAt: { select: { id: true, name: true } } }
        });
        ...
        if (
          ...
          const firstTx = await prisma.stockTransaction.findFirst({
            ...
            select: { createdAt: { select: { id: true, name: true } } }
          });
