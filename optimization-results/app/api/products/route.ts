...
export async function GET(request: NextRequest) {
  try {
    ...
    // Optimize query based on fullDetails flag - always use select for better performance
    const selectConfig = fullDetails ? {
      id: { select: { id: true, name: true } },
      name: { select: { id: true, name: true } },
      type: { select: { id: true, name: true } },
      sku: { select: { id: true, name: true } },
      description: { select: { id: true, name: true } },
      productDescription: { select: { id: true, name: true } },
      image: { select: { id: true, name: true } },
      brochure: { select: { id: true, name: true } },
      enableStock: { select: { id: true, name: true } },
      alertQuantity: { select: { id: true, name: true } },
      purchasePrice: { select: { id: true, name: true } },
      sellingPrice: { select: { id: true, name: true } },
      marginPercentage: { select: { id: true, name: true } },
      weight: { select: { id: true, name: true } },
      preparationTime: { select: { id: true, name: true } },
      enableProductInfo: { select: { id: true, name: true } },
      notForSelling: { select: { id: true, name: true } },
      ...
      unit: {
        select: {
          id: { select: { id: true, name: true } },
          name: { select: { id: true, name: true } },
          shortName: { select: { id: true, name: true } }
        }
      },
      tax: {
        select: {
      ...
      variations: {
        ...
        select: {
          ...
          purchasePrice: { select: { id: true, name: true } },
          sellingPrice: { select: { id: true, name: true } },
          isDefault: { select: { id: true, name: true } },
          subSku: { select: { id: true, name: true } },
      ...
      if (locations.length > 0) {
        ...
        if (inventoryRecords.length > 0) {
          await tx.variationLocationDetails.createMany({
            ...
            skipDuplicates: { select: { id: true, name: true } }
          })
