  ...
  return withIdempotency(request, `/api/transfers/${id}/send`, async () => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedTransfer = await tx.stockTransfer.update({
        ...
        data: {
          status: 'in_transit',
          stockDeducted: { select: { id: true, name: true } }, // CRITICAL FLAG
          sentBy: userIdNumber,
          sentAt: new Date(),
        },
      })
    ...
    try {
      ...
      if (fromLocation && toLocation) {
        ...
        const itemsWithNames = await prisma.stockTransferItem.findMany({
          ...
          select: {
            product: { select: { name: { select: { id: { select: { id: true, name: true } }, name: { select: { id: true, name: true } } } } } },
            productVariation: { select: { name: { select: { id: true, name: true } } } }
          },
          take: 3
        })
